...
def tracker():
    ...
    metadata1 = ModelMetadata(id=model_id1, block=100)
    model_id2 = ModelId(namespace="test_ns", name="test_name2", commit="c2", hash="h2", competition_id=1)
    metadata2 = ModelMetadata(id=model_id2, block=101)
    
    tracker.on_model_updated("hotkey1", metadata1)
    tracker.on_model_updated("hotkey2", metadata2)
    return tracker

def test_on_hotkeys_updated(tracker):
    """Tests that the tracker correctly removes hotkeys that are no longer active."""
    assert tracker.get_model_metadata_for_miner_hotkey("hotkey1") is not None
    assert tracker.get_model_metadata_for_miner_hotkey("hotkey2") is not None
    
    tracker.on_hotkeys_updated({"hotkey1"})
    
    assert tracker.get_model_metadata_for_miner_hotkey("hotkey1") is not None
    assert tracker.get_model_metadata_for_miner_hotkey("hotkey2") is None

def test_on_model_updated(tracker):
    """Tests that model updates are correctly stored."""
    model_id = ModelId(namespace="test_ns", name="test_name1", commit="c1", hash="h1", competition_id=1)
    metadata = ModelMetadata(id=model_id, block=100)
    assert tracker.get_model_metadata_for_miner_hotkey("hotkey1") == metadata
...
def test_save_and_load_state(tracker, temp_dir):
    ...
    assert new_tracker.get_model_metadata_for_miner_hotkey("hotkey1") == tracker.get_model_metadata_for_miner_hotkey("hotkey1")
    ...
    assert new_tracker.get_model_metadata_for_miner_hotkey("hotkey2") == tracker.get_model_metadata_for_miner_hotkey("hotkey2")
    # Check that a non-existent hotkey is still not there
    assert new_tracker.get_model_metadata_for_miner_hotkey("hotkey3") is None
