...
def test_get_local_miners_dir():
    assert utils.get_local_miners_dir("base") == "base/models"

def test_get_local_miner_dir():
    assert utils.get_local_miner_dir("base", "hotkey1") == "base/models/hotkey1"

def test_get_local_model_dir():
    ...
    assert utils.get_local_model_dir("base", "hotkey1", model_id) == expected_path

def test_get_local_model_snapshot_dir():
    model_id = ModelId(namespace="ns", name="name", commit="commit", hash="hash", competition_id=1)
    expected_path = os.path.join("base", "models", "hotkey1", "models--ns--name", "snapshots", "commit")
    assert utils.get_local_model_snapshot_dir("base", "hotkey1", model_id) == expected_path

def test_get_newest_datetime_under_path(temp_dir):
    ...
    assert newest_time.timestamp() == pytest.approx(path2.stat().st_mtime, abs=1)

def test_get_newest_datetime_empty_dir(temp_dir):
    test_path = os.path.join(temp_dir, "empty_dir")
    os.makedirs(test_path)
    assert utils.get_newest_datetime_under_path(test_path) == datetime.datetime.min
    
def test_remove_dir_out_of_grace_by_datetime(temp_dir):
    test_path = os.path.join(temp_dir, "test_dir_grace")
    os.makedirs(test_path)

    # Test removal
    last_modified_old = datetime.datetime.now() - datetime.timedelta(seconds=100)
    assert utils.remove_dir_out_of_grace_by_datetime(test_path, 50, last_modified_old)
    ...
    assert not utils.remove_dir_out_of_grace_by_datetime(test_path, 50, last_modified_new)
    assert os.path.exists(test_path)

def test_remove_dir_out_of_grace(temp_dir):
    # Create a directory with a file.
    test_path = os.path.join(temp_dir, "test_dir_grace_2")
    os.makedirs(test_path)
    ...
    # Test removal
    assert utils.remove_dir_out_of_grace(test_path, 50)
    assert not os.path.exists(test_path)
    ...
    # Test no removal
    assert not utils.remove_dir_out_of_grace(test_path, 50)
    assert os.path.exists(test_path)

def test_realize_symlinks_in_directory(temp_dir):
